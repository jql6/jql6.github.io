...
Exercises from this book by Michael Nielsen:
...
![Bitwise digit neural network diagram (image taken from the textbook)](Bitwise.png)
...
![Graph of binary entropy at y = 0.5](C(a) graph.png)
...
The full cost function equation is actually $-\sum_j(y_j \text{ln}(a_j^L))$ from dontloo's post \href{https://stats.stackexchange.com/questions/223799/different-definitions-of-the-cross-entropy-loss-function/224491#224491}{here}. We know that all $y$'s are 0 unless it's our desired class, and so we get $0 + 0 + 0 + ... + \text{ln} a_y^L + 0 + ...$ where $a_y^L$ is the activation that is multiplied by 1 instead of 0. Therefore, we will derive $y_j \text{ln}(a_j^L)$ with respect to $b_j^L$.
...
I'm not really clear on this one, I've managed to search up \href{https://cs.stackexchange.com/questions/74561/what-are-the-challenges-of-using-gradient-descent-on-hyper-parameters-\%CE\%BB-and-\%CE\%B7-to}{a few explanations} though.
...
I found a study called \href{https://arxiv.org/abs/1505.03654}{Neural Network with Unbounded Activation Functions is Universal Approximator}, but I don't understand it yet. The paper seems to talk a lot about Ridgelet transformations.
